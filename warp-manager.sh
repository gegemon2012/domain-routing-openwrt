...
is_valid_ipv4() {
    ...
    local addr part o1 o2 o3 o4
    IFS='/' read -r addr _ <<<"$ip"
...
CONF
    ...
    chmod 600 "$WARP_CONF" 2>/dev/null || true
    source "$WARP_CONF"
...
log_action() {
    touch "$WARP_LOG" 2>/dev/null || true
    chmod 600 "$WARP_LOG" 2>/dev/null || true
...
awg_load_clients() {
    ...
    if [ -n "$raw" ]; then
        while IFS= read -r line; do
            ...
            [ -n "$line" ] && is_valid_ipv4 "$line" && AWG_SELECTED_IPS+=("$line")
        done <<< "$raw"
    fi
}
...
awg_get_client_ips() {
    ...
    if [ "${#AWG_CLIENT_IPS[@]}" -eq 0 ]; then
        mapfile -t AWG_CLIENT_IPS < <(docker exec "$CONTAINER" sh -c "awk '/^

\[Peer\]

/,/^$/' '$AWG_VPN_CONF' | sed -n 's/^AllowedIPs[[:space:]]*=[[:space:]]*//p'" 2>/dev/null | tr -d '\r' | grep '/32')
    fi
    # фильтрация на валидные IP
    local filtered=()
    local ip
    for ip in "${AWG_CLIENT_IPS[@]}"; do
        is_valid_ipv4 "$ip" && filtered+=("$ip")
    done
    AWG_CLIENT_IPS=("${filtered[@]}")
}
        ...
        cl)
            ...
            if [ ${#AWG_CLIENT_IPS[@]} -eq 0 ]; then
                ...
                for i in "${!AWG_CLIENT_IPS[@]}"; do
                    ...
                    t+="\n"
                    local safe_label; safe_label=$(echo "$label" | sed 's/["\\]
...
bot_daemon() {
    ...
    chmod 600 "$BOT_PID_FILE" 2>/dev/null || true
    source "$WARP_CONF"
...
run_startup() {
    ...
    if [ "$(readlink -f "$0" 2>/dev/null)" != "/usr/local/bin/gowarp" ]; then
        # безопасное обновление: только копирование текущего проверенного скрипта
