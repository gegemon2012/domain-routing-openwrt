...
# ПРЕДУПРЕЖДЕНИЕ!
Представленные на данной странице ресурсы (ссылки, инструкции, скрипты и т.д.) могут использоваться только в ознакомительных целях. Также они могут использоваться в тестовых целях для обхода блокировок доступа к тем сайтам, которые намеренно заблокировали доступ к своему ресурсу пользователей относящихся к определенной геолокации. Иное применение настоящих ресурсов (в том числе но не исключая случаев обхода блокировок установленных действующим законодательством пользователей относящихся к определенной геолокации) - недопустимо. Если вы намеренны пользоваться настоящими ресурсами в иных, не разрешенных действующим законодательством вашей геолокации целей, то вы, используя настоящие ресурсы в запрещенных законом случаях, должны отказаться от использования настоящих ресурсов, а в случае привлечения вас к ответственности - берете на себя все возможные риски, в том числе, но не исключая - все возможные убытки и расходы.
...
# Скрипт для оптимизации работы роутера (IPv6 del + irqbalance + zram swap + watchdog freq/time + HFO & SFO)
```
...
# Скрипт для установки Рекурсивного DNS + HTTPS DNS Proxy + Quad9 (для Ruantiblock и Get domains AWG 2.0)
```
sh <(wget -O - https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/Recursive-DNS-install.sh)
```

# Скрипт для установки ZeroTier (test)
```
...
# Скрипт для удаления ruantiblock
```
sh <(wget -O - https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/ruantiblock_2_autouninstall.sh)
```

# Скрипт для установки TOR Snowflake для ruantiblock (test)
```
sh <(wget -O - https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/ruantiblock-tor-snowflake.sh)
```

# Скрипт для установки WARP Manager (test)
```
bash <(curl -fsSL https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/warp-manager.sh)
```
...
### Запуск
```
wget -O - https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/getdomains-check.sh | sh
```
...
```
wget -O - https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/getdomains-check.sh | sh -s --lang en
```

### Запустить с проверкой на подмену DNS
```
wget -O - https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/getdomains-check.sh | sh -s dns
```
...
```
wget -O - https://raw.githubusercontent.com/gegemon2012/domain-routing-openwrt/refs/heads/master/getdomains-check.sh | sh -s dump
```
...
Для **country** три [возможных значения](https://github.com/itdoginfo/allow-domains):
- russia-inside
- russia-outside
- ukraine
