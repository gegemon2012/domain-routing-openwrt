...
add_dns_resolver() {
    echo "Configure DNSCrypt2, Stubby, Recursive DNS или HTTPS DNS Proxy?"
    echo "It does matter if your ISP is spoofing DNS requests"
    ...
    echo "3) Stubby (36K)"
    echo "4) Recursive DNS (Unbound + DNSSEC)"
    echo "5) HTTPS DNS Proxy (DoH, Cloudflare)"
    ...
    # === RECURSIVE DNS (Unbound + DNSSEC) ===
    if [ "$DNS_RESOLVER" == 'UNBOUND' ]; then
        printf "\033[32;1mУстановка Recursive DNS (Unbound + DNSSEC)\033[0m\n"
        opkg install unbound-daemon luci-app-unbound bind-dig drill

        # Останавливаем всё, что может конфликтовать
        ...
        killall dnsmasq unbound https-dns-proxy dnscrypt-proxy stubby 2>/dev/null || true
        sleep 2

        # Настройка Unbound (порт 5353)
        uci -q delete unbound.@unbound[0] || true
        uci set unbound.@unbound[0]=unbound
        uci set unbound.@unbound[0].listen_port='5353'
        uci set unbound.@unbound[0].dhcp_link='none'
        uci set unbound.@unbound[0].domain_type='static'
        uci set unbound.@unbound[0].validator='1'           # DNSSEC
        uci set unbound.@unbound[0].rebind_protection='1'
        uci set unbound.@unbound[0].forward_upstream='0'    # чистая рекурсия
        ...
        printf "\033[32;1mUnbound + DNSSEC успешно настроен (порт 5353)\033[0m\n"
    fi

    # === HTTPS DNS PROXY (DoH) ===
    if [ "$DNS_RESOLVER" == 'HTTPS_DNS_PROXY' ]; then
        printf "\033[32;1mУстановка HTTPS DNS Proxy (DoH, Cloudflare)\033[0m\n"
        opkg install https-dns-proxy luci-app-https-dns-proxy

        # Останавливаем конфликты
        /etc/init.d/dnsmasq stop 2>/dev/null
        /etc/init.d/https-dns-proxy stop 2>/dev/null
        /etc/init.d/unbound stop 2>/dev/null
        /etc/init.d/dnscrypt-proxy stop 2>/dev/null
        /etc/init.d/stubby stop 2>/dev/null
        killall dnsmasq https-dns-proxy unbound dnscrypt-proxy stubby 2>/dev/null || true
        sleep 2

        # Настройка https-dns-proxy (Cloudflare DoH)
        uci -q delete https-dns-proxy.dns
        ...
        printf "\033[32;1mHTTPS DNS Proxy (DoH Cloudflare) настроен на порт 5053\033[0m\n"
    fi
}
