...
$progress.Location = New-Object System.Drawing.Point(10, 110)
$progress.Width = 400
$progress.Height = 20
...
$btnBackup.Location = New-Object System.Drawing.Point(450, 10)
$btnBackup.Width = 220
$btnBackup.Height = 40
...
$btnSafeRestore.Location = New-Object System.Drawing.Point(450, 55)
...
$btnFullRestore.Location = New-Object System.Drawing.Point(450, 95)
...
$btnSetEmptyPassword = New-Object System.Windows.Forms.Button
$btnSetEmptyPassword.Text = "Set Empty Password"
$btnSetEmptyPassword.Location = New-Object System.Drawing.Point(450, 135)
$btnSetEmptyPassword.Width = 220
$btnSetEmptyPassword.Height = 25
$btnSetEmptyPassword.BackColor = "Orange"
$btnSetEmptyPassword.Font = New-Object System.Drawing.Font("Microsoft Sans Serif", 8, [System.Drawing.FontStyle]::Bold)
$panel.Controls.Add($btnSetEmptyPassword)

$btnClose = New-Object System.Windows.Forms.Button
$btnClose.Text = "Close"
$btnClose.Location = New-Object System.Drawing.Point(450, 165)
$btnClose.Width = 220
$btnClose.Height = 25
...
function Set-EmptyPassword {
    Write-Log ""
    Write-Log "=========================================="
    Write-Log "SETTING EMPTY PASSWORD FOR ROOT"
    Write-Log "=========================================="
    
    $ip = $txtIP.Text
    $user = $txtUser.Text
    
    Write-Log "This will set an EMPTY password for user '$user'"
    Write-Log "After this, you can login WITHOUT any password"
    Write-Log "This is the default OpenWRT state after clean installation"
    Write-Log ""
    
    $confirm = [System.Windows.Forms.MessageBox]::Show(
        "Set empty password for $user@$ip?`n`nAfter this, you can login WITHOUT any password.`n`nContinue?",
        "Set Empty Password",
        "YesNo",
        "Question"
    )
    ...
    Write-Log "Setting empty password on router..."
    Write-Log ">>> Enter CURRENT password when prompted <<<"
    Write-Log ""
    
    $result = & ssh -o StrictHostKeyChecking=no $user@$ip "echo -e '\n\n' | passwd" 2>&1
    ...
    Write-Log ""
    Write-Log "=========================================="
    Write-Log "EMPTY PASSWORD SET"
    Write-Log "=========================================="
    Write-Log "You can now login without password"
    Write-Log "Try: ssh -o StrictHostKeyChecking=no $user@$ip"
    Write-Log ""
    
    [System.Windows.Forms.MessageBox]::Show(
        "Empty password set for $user@$ip!`n`nYou can now login without any password.`n`nTry: ssh $user@$ip", 
        "Empty Password Set", "OK", "Information"
    )
}
...
function Create-Backup {
    ...
    # Выбираем папку для сохранения бэкапа
    $folder = New-Object System.Windows.Forms.FolderBrowserDialog
    $folder.Description = "Select folder to save backup"
    $folder.ShowNewFolderButton = $true
    
    if ($folder.ShowDialog() -ne "OK") {
        Write-Log "Backup cancelled by user"
        return
    }
    
    $backupSavePath = $folder.SelectedPath
    Write-Log "Backup will be saved to: $backupSavePath"
    
    $ip = $txtIP.Text
    $user = $txtUser.Text
    
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupDir = "$backupSavePath\OpenWRT_Backup_$timestamp"
    ...
    $mtdList = & ssh -o StrictHostKeyChecking=no $user@$ip "cat /proc/mtd" 2>&1
    ...
    foreach ($part in $partitions) {
        ...
        $dumpContent = & ssh -o StrictHostKeyChecking=no $user@$ip "dd if=/dev/$($part.Device) 2>/dev/null" 2>&1
...
function Restore-Backup {
    ...
    $currentMtd = & ssh -o StrictHostKeyChecking=no $user@$ip "cat /proc/mtd" 2>&1
    ...
    if (-not $SkipDangerous) {
        $confirm2 = [System.Windows.Forms.MessageBox]::Show(
            "FINAL WARNING!`n`nYou are about to restore dangerous partitions!`nThis can permanently damage your router!`n`nClick YES to continue",
    ...
    foreach ($item in $restoreList) {
        ...
        if ($backupData) {
            ...
            $result = & ssh -o StrictHostKeyChecking=no $user@$ip "echo '$escapedData' | dd of=/dev/$($item.Device) 2>/dev/null && echo 'OK'" 2>&1
    ...
    if ($reboot -eq "Yes") {
        ...
        & ssh -o StrictHostKeyChecking=no $user@$ip "reboot" 2>&1
...
Write-Log "  - Click 'Create Backup' and select folder"
Write-Log "  - Creates full backup of ALL MTD partitions"
Write-Log ""
Write-Log "SAFE RESTORE:"
Write-Log "  - Click 'Safe Restore' and select backup folder"
...
Write-Log "  - Click 'Full Restore' and select backup folder"
Write-Log "  - Restores ALL partitions including dangerous"
Write-Log "  - Can brick your router!"
Write-Log ""
Write-Log "SET EMPTY PASSWORD:"
Write-Log "  - Sets empty password for root (default OpenWRT state)"
Write-Log ""
